...
resource "vsphere_virtual_machine" "vm_1" {
  ...
  provisioner "file" {
      ...
      content = <<EOF
#!/bin/bash
echo ${var.DB_Server_IP} > /root/db2ip.txt
sh /root/ibm-datasrvrmgr/bin/start.sh

echo "done"

EOF

  destination = "/root/start_db2mgt.sh"

  }
  provisioner "remote-exec" {
    inline = [
      "chmod +x /root/start_db2mgt.sh; bash /root/start_db2mgt.sh"
    ]
  }
  provisioner "file" {
